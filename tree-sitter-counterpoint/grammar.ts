...
module.exports = grammar({
	...
	rules: {
		...
		...parameterize('type_tuple_literal',  ({variable}) => $ => seq('[', optional(seq(OPT_COM, $[call('_items_type',      {variable})])), ']'), 'variable'),
		...parameterize('type_record_literal', ({variable}) => $ => seq('[',              OPT_COM, $[call('_properties_type', {variable})],   ']'), 'variable'),
		...
		...parameterize('_type_unit', ({variable}) => $ => choice(
			...
			$[call('type_tuple_literal',  {variable})],
			$[call('type_record_literal', {variable})],
			$.type_dict_literal,
			$.type_map_literal,
		), 'variable'),
		...
		...parameterize('tuple_literal',      ({variable}) => $ => seq('[', optional(seq(OPT_COM, repCom1($[call('_expression', {variable})]), OPT_COM)), ']'), 'variable'),
		...parameterize('record_literal',     ({variable}) => $ => seq('[',              OPT_COM, repCom1($[call('property',    {variable})]), OPT_COM,   ']'), 'variable'),
		...
		...parameterize('_expression_unit', ({variable}) => $ => choice(
			...
			$[call('tuple_literal',      {variable})],
			$[call('record_literal',     {variable})],
			$.set_literal,
			$.map_literal,
		), 'variable'),

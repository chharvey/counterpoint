	...
	describe('#decorateTS', () => {
		...
		new Map<string, readonly [ConstructorType<AST.ASTNodeCP>, string]>([
			...
			['Decorate(ExpressionCompound<Block, Break> ::= ExpressionCompound<?Block><?Break> "?." PropertyAccessor<?Break>) -> SemanticAccess', [AST.ASTNodeAccess, `
				...
				% (expression_compound)
			`]],
			['skip: Decorate(ExpressionCompound<Block, Break> ::= ExpressionCompound<?Block><?Break> "!." PropertyAccessor<?Break>) -> SemanticAccess', [AST.ASTNodeAccess, `
				{
					v!.p;
				}
				% (expression_compound)
			...
			...['<', '>', '<=', '>=', '!<', '!>', 'is', 'isnt'].map((op) => [`${ ['is', 'isnt'].includes(op) ? 'skip: ' : '' }Decorate(ExpressionComparative<Block, Break> ::= ExpressionComparative<?Block><?Break> "${ op }" ExpressionAdditive<?Block><?Break>) -> SemanticOperation`, [AST.ASTNodeOperation, `
				{
					a ${ op } b;
				}

...
describe('ASTNodeTypeAccess', () => {
	describe('#eval', () => {
		...
		context('index access.', () => {
			it('returns individual entry types.', () => {
				assert.deepStrictEqual(
					program.block!.children.slice(2, 8).map((c) => evalTypeDecl(c)),
					expected,
				);
			});
			it('negative indices count backwards from end.', () => {
				assert.deepStrictEqual(
					program.block!.children.slice(8, 14).map((c) => evalTypeDecl(c)),
					expected,
				);
			});
			it('unions with void if entry is optional.', () => {
				assert.deepStrictEqual(
					program.block!.children.slice(24, 26).map((c) => evalTypeDecl(c)),
					[
		...
		context('key access.', () => {
			it('returns individual entry types.', () => {
				assert.deepStrictEqual(
					program.block!.children.slice(16, 22).map((c) => evalTypeDecl(c)),
					expected,
				);
			});
			it('unions with void if entry is optional.', () => {
				assert.deepStrictEqual(
					program.block!.children.slice(28, 30).map((c) => evalTypeDecl(c)),
					[

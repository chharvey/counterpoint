	...
	describe('#codegen', () => {
		...
		describe('CollectionLinearNew', () => {
			...
			it('LIST.NEW returns (struct.new) with count and internal array.', () => {
				...
				const mod = cg.module;
				const ht_list_internal: binaryen.Type = cg.getHeapType('$ListInternal')!;
				const ht_list:          binaryen.Type = cg.getHeapType('$List')!;
				return assertEqualBins(
					(goal.children[1] as AST.ASTNodeStatementExpression).expr!.lower(opt).codegen(cg),
					mod.block(null, [
						mod.local.set(3, mod.array.new_default(ht_list_internal, mod.i32.const(8))),
						...[
						...
						].map((code, i) => mod.array.set(mod.local.get(3, ht_list_internal), mod.i32.const(i), code)),
						mod.struct.new([
							new BinVect(mod, mod.i32.const(5)).vect,
							mod.local.get(3, ht_list_internal),
						], ht_list),
					], ht_list),
				);
			});
		...
		it('DICT.NEW returns (struct.new) with count and internal array.', () => {
			...
			// @ts-expect-error --- WASM 3.0 (incl. GC) not typed yet
			// eslint-disable-next-line
			const WASM_NULL: binaryen.ExpressionRef = mod.ref.null(binaryen.getTypeFromHeapType(cg.getHeapType('$DictEntry')!, true));
			return assertEqualBins(
				(goal.children[1] as AST.ASTNodeStatementExpression).expr!.lower(opt).codegen(cg),
				mod.block(null, [
					mod.local.set(3, mod.array.new_default(ht_dict_internal, mod.i32.const(8))),
					...[
						WASM_NULL,
						...
						WASM_NULL,
						WASM_NULL,
					].map((code, i) => mod.array.set(mod.local.get(3, ht_dict_internal), mod.i32.const(i), code)),
					mod.struct.new([
						new BinVect(mod, mod.i32.const(5)).vect,
						mod.local.get(3, ht_dict_internal),
					], ht_dict),
				], ht_dict),
			);
		});

	...
	describe('#constructor', () => {
		context('no args', () => {
			it('prints the help message.', () => {
				const cli: CLI = new CLI('npx cpc'.split(' '));
		...
		context('--help', () => {
			it('same as `-h`; prints help message.', () => {
				const cli: CLI = new CLI('npx cpc -h'.split(' '));
		...
		context('--help --config', () => {
			it('prints help text and config options.', () => {
				const cli: CLI = new CLI('npx cpc -h --config'.split(' '));
				assert.strictEqual(cli.argv.help, true);
		...
		context('--version', () => {
			it('same as `-v`; prints version number.', () => {
				const cli: CLI = new CLI('npx cpc -v'.split(' '));
		...
		context('compile --out', () => {
			it('same as `-o`; compiles given file to specified output.', () => {
				const cli: CLI = new CLI('npx cpc compile ./sample/test-v0.1.cp  --out ./sample/testout.wasm'.split(' '));
				assert.strictEqual(cli.argv._[1], './sample/test-v0.1.cp');
		...
		context('compile --project', () => {
			it('same as `-p`; compiles given file with the specified project settings.', () => {
				const cli: CLI = new CLI('npx cpc compile ./sample/test-v0.1.cp  --project ./sample/counterpoint-config.json'.split(' '));
				assert.strictEqual(cli.argv._[1], './sample/test-v0.1.cp');
		...
		context('dev --out', () => {
			it('same as `-o`; debugs given file to specified output.', () => {
				const cli: CLI = new CLI('npx cpc dev ./sample/test-v0.1.cp  --out ./sample/testout.wat'.split(' '));
				assert.strictEqual(cli.argv._[1], './sample/test-v0.1.cp');
		...
		context('dev --project', () => {
			it('same as `-p`; debugs given file with the specified project settings.', () => {
				const cli: CLI = new CLI('npx cpc dev ./sample/test-v0.1.cp  --project ./sample/counterpoint-config.json'.split(' '));
				assert.strictEqual(cli.argv._[1], './sample/test-v0.1.cp');
				assert.strictEqual(cli.argv.project, './sample/counterpoint-config.json');
				assert.strictEqual(cli.argv.project, cli.argv.p);
			});

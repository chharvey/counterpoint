...
### Type 1: Integer
Type 1 is uninterpreted integer data, representing a general numerical value.
...
### Type 2: Float
Type 2 encodes floating-point numbers in the an *IEEE 754-2019* standard format.
An `objtype` of `\x22` represents a 16-bit float and has a payload of 2 bytes of data.
`\x24` and `\x28` types represent 32-bit and 64-bit floats, having a payload of 4 and 8 bytes of data respectively.
Padding is prepended to the `body` as necessary to increase it to a multiple of 8.

#### Type 2 Validation
```
Validate(Type2 ::= ‹refcount› \x28                              ‹reserved› \x00 \x00 \x00 \x08             P{8}    R* ␃) -> Validate(R* ␃);
Validate(Type2 ::= ‹refcount› \x28                              ‹reserved› \x00 \x00 \x00 \x08 \x00{0, 7}  P{0, 7} R* ␃) -> false;
Validate(Type2 ::= ‹refcount› \x24                              ‹reserved› \x00 \x00 \x00 \x04 \x00{0, 3}  P{5, 8} R* ␃) -> false;
Validate(Type2 ::= ‹refcount› \x24                              ‹reserved› \x00 \x00 \x00 \x04 \x00{4}     P{4}    R* ␃) -> Validate(R* ␃);
Validate(Type2 ::= ‹refcount› \x24                              ‹reserved› \x00 \x00 \x00 \x04 \x00{5, 7}? P{0, 3} R* ␃) -> false;
Validate(Type2 ::= ‹refcount› \x22                              ‹reserved› \x00 \x00 \x00 \x02 \x00{0, 5}  P{3, 8} R* ␃) -> false;
Validate(Type2 ::= ‹refcount› \x22                              ‹reserved› \x00 \x00 \x00 \x02 \x00{6}     P P     R* ␃) -> Validate(R* ␃);
Validate(Type2 ::= ‹refcount› \x22                              ‹reserved› \x00 \x00 \x00 \x02 \x00{7}?    P?      R* ␃) -> false;
Validate(Type2 ::= ‹refcount› [\x20-\x21\x23\x25-\x27\x29-\x2f] ‹reserved›                                         R* ␃) -> false;
```


### Type 3: Address
Type 3 represents an address as an “offset”. It has an `objtype` of `\x38`.
...
#### Type 3 Validation
```
Validate(Type3 ::= ‹refcount› \x38                 ‹reserved› \x00 \x00 \x00 \x08            P{8}    R* ␃) -> Validate(R* ␃);
Validate(Type3 ::= ‹refcount› \x38                 ‹reserved› \x00 \x00 \x00 \x08 \x00{0, 7} P{0, 7} R* ␃) -> false;
Validate(Type3 ::= ‹refcount› [\x30-\x37\x39-\x3f] ‹reserved›                                        R* ␃) -> false;
```


### Type 4: String
Type 4 is textual data (a “string” of characters) encoded in [UTF-8 format](https://en.wikipedia.org/wiki/UTF-8).
It has an `objtype` of `\x40`. Padding is prepended to the `body` as necessary to increase it to a multiple of 8.
...
```
\xff \xff \x40 \x00 \x00 \x00 \x00 \x05 \x00 \x00 \x00 \x68 \x65 \x6c \x6c \x6f
...
#### Type 4 Validation
```
Validate(Type4 ::=‹refcount› \x40        ‹reserved› C{4}    \x00{0, 7} P{numeric(C{4})}        R* ␃) -> UTF8Validate(P{numeric(C{4})}) && Validate(R* ␃);
Validate(Type4 ::=‹refcount› \x40        ‹reserved› C{4}    \x00{0, 7} P{0, numeric(C{4}) - 1} R* ␃) -> false;
Validate(Type4 ::=‹refcount› \x40        ‹reserved› C{0, 3}                                    R* ␃) -> false;
Validate(Type4 ::=‹refcount› [\x41-\x4f] ‹reserved›                                            R* ␃) -> false;
```


### Type 5: Binary
Type 5 type stores arbitrary unstructured binary data.
It is similar in format to Type 4, except that the payload need not be valid in UTF-8.
Binary data may be mutated.

#### Type 5 Validation
```
Validate(Type5 ::= ‹refcount› \x50        ‹reserved› C{4}    \x00{0, 7} P{numeric(C{4})}        R* ␃) -> Validate(R* ␃);
Validate(Type5 ::= ‹refcount› \x50        ‹reserved› C{4}    \x00{0, 7} P{0, numeric(C{4}) - 1} R* ␃) -> false;
Validate(Type5 ::= ‹refcount› \x50        ‹reserved› C{0, 3}                                    R* ␃) -> false;
Validate(Type5 ::= ‹refcount› [\x51-\x5f] ‹reserved›                                            R* ␃) -> false;
```


### Type 6: Struct
Type 6 (`objtype` `\x60`) represents structs (“structures”), which have a length and a (possibly) heterogeneous mix of items.
...
As with Type 4, this may differ from the struct’s **count**.
...
Structs may only contain objects of Type 0–3. For other types, references (addresses, see Type 3) must be used instead.
...
```
\xff \xff \x60 \x00 \x00 \x00 \x00 \x0d \x00 \x00 \x00 \x03 \x12 \x00 \x06 \x38 \x00 \x00 \x00 \x00 \x00 \x00 \x01 \x30
...
```
\xff \xff \x60 \x00 \x00 \x00 \x00 \x00
|         |    |    |- 4–7: `bytecount`
|         |    |- 3: reserved
|         |- 2: `objtype`
|- 0–1: `refcount`
...
When structs refer to Types 4–7 objects, those objects must be stored separately in the heap, and their addresses are then held in the struct.
...
```
\xff \xff \x60 \x00 \x00 \x00 \x00 \x12 \x00 \x00 \x00 \x00 \x00 \x00 \x38 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x20 \x38 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x30
...
(continued)
\x00 \x01 \x40 \x00 \x00 \x00 \x00 \x05 \x00 \x00 \x00 \x68 \x65 \x6c \x6c \x6f
...
(continued)
\x00 \x01 \x40 \x00 \x00 \x00 \x00 \x05 \x00 \x00 \x00 \x77 \x6f \x72 \x6c \x64
...
```
\xff \xff \x60 \x00 \x00 \x00 \x00 \x1b \x00 \x00 \x00 \x00 \x00 \x38 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x28 \x38 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x30 \x38 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x50
...
(continued)
\x00 \x01 \x60 \x00 \x00 \x00 \x00 \x00
|         |    |    |- 44–47: `bytecount`
|         |    |- 43: reserved
|         |- 42: `objtype`
...
(continued)
\x00 \x01 \x60 \x00 \x00 \x00 \x00 \x09 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x38 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x48     \x00 \x01 \x60 \x00 \x00 \x00 \x00 \x00
...
(continued)
\x00 \x01 \x60 \x00 \x00 \x00 \x00 \x12 \x00 \x00 \x00 \x00 \x00 \x00 \x38 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x70 \x38 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x78     \x00 \x01 \x60 \x00 \x00 \x00 \x00 \x00     \x00 \x01 \x60 \x00 \x00 \x00 \x00 \x09 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x38 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x90     \x00 \x01 \x60 \x00 \x00 \x00 \x00 \x00
...
#### Type 6 Validation
```
Validate(Type6 ::= ‹refcount› \x60        ‹reserved› C{4}    \x00{0, 7} P{numeric(C{4})}        R* ␃) -> StructValidate(P{numeric(C{4})}) && Validate(R* ␃);
Validate(Type6 ::= ‹refcount› \x60        ‹reserved› C{4}    \x00{0, 7} P{0, numeric(C{4}) - 1} R* ␃) -> false;
Validate(Type6 ::= ‹refcount› \x60        ‹reserved› C{0, 3}                                    R* ␃) -> false;
Validate(Type6 ::= ‹refcount› [\x61-\x6f] ‹reserved›                                            R* ␃) -> false;
...
StructValidate(StructPayload ::= [\x30-\x37\x39-\x3f]                      R*) -> false;
StructValidate(StructPayload ::= [\x40-\xff]                               R*) -> false;
```


### Type 7: Array
Type 7 (`objtype` `\x70`) represents arrays, which have a length and a homogeoneous item type.
...
This differs from Type 6, which has no empty slots.

An array’s *count* must be the least power of 2 greater than or equal to the number of items it holds at any given time;
for example, an array holding 5 items must have a count of 8.
The slots in an array may be filled and unfilled as needed, as long as the array’s type does not change.
However, unlike Type 6 objects, Type 7 objects may change their length —
...
(E.g., after 8 items, the new the array’s count must be 16, and then 32, etc.)
...
a 16-count array must be moved into an 8-count array once its filled slots decrease from 9 to 8.
...
Arrays may only contain objects of Type 0–3, and they all must be the same type.
For other types, or a heterogeneous mix of types, references (addresses) must be used instead — see Type 3.

Since arrays are homogeneous, there is no need to encode metadata for each item in the array individually as is done in Type 6.
...
Since `objtype` differs for all Type 0 items, this metadata byte is `\x80` (so as not to be confused with padding),
...
```
\xff \xff \x70 \x00 \x00 \x00 \x00 \x09 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x80 \x01 \x02 \x03 \x02 \x01 \x00 \x00 \x00
|         |    |    |                   |                                  |    |                        |- 21–23: default bytes due to sparseness
|         |    |    |                   |                                  |    |- 16–20: objects’ data (excluding `objtype`s)
|         |    |    |                   |                                  |- 15: items’ type (Type 0, represented by `\x80`)
...
```
\xff \xff \x70 \x00 \x00 \x00 \x00 \x09 \x00 \x00 \x00 \x00 \x00 \x00 \x00 \x12 \x00 \x06 \x01 \x32 \x3b \xca \x00 \x00
...
#### Type 7 Validation
```
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x80                              P{numeric(C{4}) - 1}    R* ␃) -> ArrayValidate(\x80, P{numeric(C{4}) - 1}) && Validate(R* ␃);
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x80                              P{0, numeric(C{4}) - 2} R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} [\x01-\x0f]                                               R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x18                              P{numeric(C{4}) - 1}    R* ␃) -> ArrayValidate(\x18, P{numeric(C{4}) - 1}) && Validate(R* ␃);
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x18                              P{0, numeric(C{4}) - 2} R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x14                              P{numeric(C{4}) - 1}    R* ␃) -> ArrayValidate(\x14, P{numeric(C{4}) - 1}) && Validate(R* ␃);
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x14                              P{0, numeric(C{4}) - 2} R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x12                              P{numeric(C{4}) - 1}    R* ␃) -> ArrayValidate(\x12, P{numeric(C{4}) - 1}) && Validate(R* ␃);
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x12                              P{0, numeric(C{4}) - 2} R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} [\x10-\x11\x13\x15-\x17\x19-\x1f]                         R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x28                              P{numeric(C{4}) - 1}    R* ␃) -> ArrayValidate(\x28, P{numeric(C{4}) - 1}) && Validate(R* ␃);
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x28                              P{0, numeric(C{4}) - 2} R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x24                              P{numeric(C{4}) - 1}    R* ␃) -> ArrayValidate(\x24, P{numeric(C{4}) - 1}) && Validate(R* ␃);
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x24                              P{0, numeric(C{4}) - 2} R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x22                              P{numeric(C{4}) - 1}    R* ␃) -> ArrayValidate(\x22, P{numeric(C{4}) - 1}) && Validate(R* ␃);
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x22                              P{0, numeric(C{4}) - 2} R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} [\x20-\x21\x23\x25-\x27\x29-\x2f]                         R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x38                              P{numeric(C{4}) - 1}    R* ␃) -> ArrayValidate(\x38, P{numeric(C{4}) - 1}) && Validate(R* ␃);
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} \x38                              P{0, numeric(C{4}) - 2} R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} [\x30-\x37\x39-\x3f]                                      R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{4}    \x00{0, 7} [\x40-\x7f\x81-ff]                                        R* ␃) -> false;
Validate(Type7 ::= ‹refcount› \x70        ‹reserved› C{0, 3}                                                                      R* ␃) -> false;
Validate(Type7 ::= ‹refcount› [\x71-\x7f] ‹reserved›                                                                              R* ␃) -> false;

ArrayValidate(\x80, ArrayPayload ::= [\x00-\x03] R*) -> ArrayValidate(\x80, R*);
ArrayValidate(\x80, ArrayPayload ::= [\x04-\x0f] R*) -> false;
...
ArrayValidate(\x38, ArrayPayload ::= P{8}        R*) -> StructValidate(\x38, R*);

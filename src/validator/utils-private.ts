...
export function isSyntaxNodeSupertype<C extends Category>(syntaxnode: SyntaxNode | null, category: C): syntaxnode is SyntaxNodeSupertype<C> {
	...
	return new Map<Category, (node: SyntaxNode) => boolean>([
		...
		['statement',   (node) => isSyntaxNodeType(node, /^declaration|statement_((expression|claim|reassignment|delete)(__break)?|conditional(__unless)?(__break)?|loop|iteration|break)$/) || isSyntaxNodeSupertype(node, 'declaration')],

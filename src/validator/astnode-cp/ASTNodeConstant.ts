...
export class ASTNodeConstant extends ASTNodeExpression {
	...
	private get value(): OBJ.Primitive {
		return this._value ??= (
			(isSyntaxNodeType(this.start_node, /^template_(full|head|middle|tail)$/)) ? (Dev.supports('literalTemplate-cook')) ? new OBJ.String(Validator.cookTokenTemplate(this.start_node.text)) : (() => { throw new Error('`literalTemplate-cook` not yet supported.'); })() :
			...
			(isSyntaxNodeType(this.start_node, 'primitive_literal'),  ((token: SyntaxNode) => (
				...
				(isSyntaxNodeType(token, /^string(__comment)?(__separator)?$/),  (Dev.supports('literalString-cook')) ? new OBJ.String(Validator.cookTokenString(token.text, this.validator.config)) : (() => { throw new Error('`literalString-cook` not yet supported.'); })())
	...
	protected override fold_do(): OBJ.Object {
		if (this.value instanceof OBJ.String && !Dev.supports('stringConstant-assess')) {

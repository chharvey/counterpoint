	...
	public decorateTS(syntaxnode: SyntaxNode): AST.ASTNodeCP {
		const decorators = new Map<string | RegExp, (node: SyntaxNode) => AST.ASTNodeCP>([
			...
			['identifier', (node) => (
				...
				(isSyntaxNodeSupertype(node.parent!, 'expression') || isSyntaxNodeType(node.parent!, /^(property|case|function_arguments|property_accessor|assignee|declaration_(variable|reassignment)|statement_expression)$/)) ? new AST.ASTNodeVariable (node as SyntaxNodeType<'identifier'>) :
			...
			['primitive_literal', (node) => (
				...
				(isSyntaxNodeSupertype(node.parent!, 'expression') || isSyntaxNodeType(node.parent!, /^(property|case|function_arguments|property_accessor|assignee|declaration_(variable|reassignment)|statement_expression)$/)) ? new AST.ASTNodeConstant    (node as SyntaxNodeType<'primitive_literal'>) :
			...
			['expression_grouped', (node) => this.decorateExprNode(node.children[1] as SyntaxNodeSupertype<'expression'>)],
			...
			['statement_conditional', (node) => node.children.length === 5 ? new AST.ASTNodeStatementConditional(
				...
				(node.children.length === 7
					? this.decorateTS(node.children[5] as SyntaxNodeType<'block'>)
					: this.decorateTS(node.children[5] as SyntaxNodeType<'statement_conditional'>)
				),
			)],

			['statement_conditional__unless', (node) => new AST.ASTNodeStatementConditional(
				node as SyntaxNodeType<'statement_conditional__unless'>,
				true,
				this.decorateExprNode(node.children[1] as SyntaxNodeSupertype<'expression'>),
				this.decorateTS(node.children[3] as SyntaxNodeType<'block'>),
			)],

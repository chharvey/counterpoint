...
export class Decorator {
	...
	static decorate(node: PARSER.ParseNodeTypeTupleLiteral):           AST.ASTNodeTypeEmptyCollection | AST.ASTNodeTypeTuple;
	...
	static decorate(node: PARSER.ParseNodeTupleLiteral):            AST.ASTNodeEmptyCollection | AST.ASTNodeTuple;
	static decorate(node: PARSER.ParseNodeTupleLiteral__0__List):   NonemptyArray<AST.ASTNodeExpression>;
	...
	static decorate(node: ParseNode): ASTNodeSolid | readonly ASTNodeSolid[] {
		if (Dev.supports('literalCollection') && node instanceof PARSER.ParseNodeWord) {
			...
			return (node.children.length === 2)
				// @ts-expect-error
				? new AST.ASTNodeTypeEmptyCollection(node)
				: new AST.ASTNodeTypeTuple(node, this.decorate(
			...
			return (node.children.length === 2)
				// @ts-expect-error
				? new AST.ASTNodeEmptyCollection(node)
				: new AST.ASTNodeTuple(node, this.decorate(
